...
db_url = 'postgresql://postgres:postgres:port/Name'
...
def retrieve_pupils_from_db():
    try:
        schueler_list = session.query(Schueler).all()
...
def retrieve_teacher_from_db():
    try:
        teacher_list = session.query(Lehrer).all()
        teacher_names = [f"{teacher.vorname} {teacher.nachname}" for teacher in teacher_list]
        teacher_data = "\n".join(teacher_names)
        teacher_entry.configure(text=teacher_data)  # Füge die aktualisierten Lehrerdaten im Button ein
